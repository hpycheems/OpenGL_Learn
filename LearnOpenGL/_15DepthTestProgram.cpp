...
int main() {
	...
	//禁用深度缓冲的写入
	//glDepthMask(GL_FALSE);//只有在深度测试被启用的时候有效果。

	//深度测试函数（修改深度测试中使用的比较运算符）
	glDepthFunc(GL_LESS);//Always(总是) NEVER（总不） Less（小于） Equal（等于） LEqual（小于等于） Greater（大于） NotEqual（不等于） GEqual（大于等于）


	//深度冲突：深度缓冲没有足够的精度来决定两个形状哪个在前面。
	//避免：永远不要把多个物体摆得太靠近，以至于它们的一些三角形会重叠。
	//尽可能将近平面设置远一些。
	//使用更高精度的深度缓冲。
