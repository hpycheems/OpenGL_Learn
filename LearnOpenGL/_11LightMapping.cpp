...
int main() {
	...
	GLFWwindow* window = glfwCreateWindow(800, 600, "PhongLightModel", NULL, NULL);
	if (!window) {
	...
	glTexParameteri(GL_TEXTURE_2D, GL_TEXTURE_MIN_FILTER, GL_LINEAR);
	glTexParameteri(GL_TEXTURE_2D, GL_TEXTURE_MAG_FILTER, GL_LINEAR);
	unsigned char* dataSpec = stbi_load("lighting_maps_specular_color.png", &width, &height, &nrChannels, 0);
	if (dataSpec) {
		glTexImage2D(GL_TEXTURE_2D, 0, GL_RGBA, width, height, 0, GL_RGBA, GL_UNSIGNED_BYTE, dataSpec);
		glGenerateMipmap(GL_TEXTURE_2D);
	}
	else {
		std::cout << "Load Texture Failed !" << "\n";
	...
	Light light("LightPosition_v.glsl", "LightPosition_f.glsl");
	...
	Shader object("LightMapping_v.glsl", "LightMapping_f.glsl");
	object.use();
	...
	while (!glfwWindowShouldClose(window)) {
		...
		object.setVec3("light.position", light.lightPosition);
		object.setVec3("light.ambient", glm::vec3(0.2f));
		object.setVec3("light.diffuse", glm::vec3(0.5f));
